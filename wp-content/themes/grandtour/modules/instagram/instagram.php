...
class instagramPhp{
    ...
    public function queryInstagram($url){
        //prepare caching
        $cachefolder = GRANDTOUR_THEMEUPLOAD.'instagram/';
        ...
        if(!file_exists($cachefile)){
            ...
            //If couldn't get data from WP file system then use alternative PHP functions
            if(empty($request))
            {
            ...
            //Cache result
			$wp_filesystem = grandtour_get_wp_filesystem();
			$cache_result = $wp_filesystem->put_contents($cachefile, $request);
