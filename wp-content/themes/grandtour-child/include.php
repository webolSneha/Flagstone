	  ...
	  while ( $loop->have_posts() ) : $loop->the_post();
	   		if( get_post_meta( get_the_ID(), 'ski_condition', true ) ){
		   		...
		   		// }else{
		   			...
		   			$country_html .= '<option value="'.get_the_ID().'" style="display:none;" class="destination-id-'. wp_get_post_parent_id(get_the_ID()) .'" >'. get_the_title() .'</option>';

		   			$resorts_html .= '<option value="'.get_the_ID().'" style="display:none;" class="destination-id-'. wp_get_post_parent_id(get_the_ID()) .'" >'. get_the_title() .'</option>';
		   		//}
	   		}
	   endwhile;
	   wp_reset_postdata();	
